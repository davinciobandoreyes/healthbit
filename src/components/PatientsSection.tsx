...
export const PatientsSection: React.FC<PatientsSectionProps> = ({ onDepthChange }) => {
  ...
  const reportDepth = (depth: 1 | 2) => onDepthChange?.(depth);
  ...
  if (isCreating) {
    return <PatientCreateView onCancel={handleCancelCreate} onSubmit={handleCreatePatient} />;
  }
      ...
      {filteredPatients.length === 0 ? (
        <div className="bg-white border border-slate-200/80 rounded-3xl p-12 text-center space-y-3 max-w-md mx-auto">
          ...
          <button
            type="button"
            onClick={openCreate}
            className="min-h-[44px] px-4 rounded-xl bg-violet-600 hover:bg-violet-700 text-white font-bold text-xs cursor-pointer"
          >
            Crear expediente
          </button>
        </div>
